...
const Sellers = () => {
  ...
  const fetchSellers = async () => {
    ...
    try {
      ...
      let url = 'http://localhost:3000/api/seller/list';
      ...
      const mappedSellers = data.sellers.map(seller => ({
        ...
        phone: seller.phone,
        store: seller.shop ? { id: seller.shop.id, name: seller.shop.name } : null,
  ...
  const fetchSeller = async (sellerId) => {
    try {
      ...
      const mappedSeller = {
        ...
        store: data.seller.shop ? { id: data.seller.shop.id, name: data.seller.shop.name } : null,
  ...
  const handleCreateSeller = async (sellerData) => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/seller/create', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
          ...
          shopId: sellerData.store_id,
          username: sellerData.username,
          password: sellerData.password
        })
      });

      // Request body ni console.log qilish
  ...
  const handleEditSeller = async (sellerData) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/seller/${sellerData.id}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleStatusChange = async (sellerId, newStatus) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/seller/${sellerId}/status`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handlePasswordChange = async (sellerId, password) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/seller/${sellerId}/password`, {
        method: 'PATCH',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
