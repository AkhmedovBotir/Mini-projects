...
const StoreOwners = () => {
  ...
  const fetchAvailablePermissions = async () => {
    try {
      const response = await fetch('http://localhost:3000/api/shop-owner/permissions', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleCreateOwner = async (data) => {
    try {
      const response = await fetch('http://localhost:3000/api/shop-owner/create', {
        method: 'POST',
        headers: {
  ...
  const handleEditOwner = async (id, data) => {
    try {
      const response = await fetch(`http://localhost:3000/api/shop-owner/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      setOwners(prevOwners => 
        prevOwners.map(owner => 
          owner.id === id ? result.shopOwner : owner
        )
      );
  ...
  const handleEditPermissions = async (id, permissions) => {
    try {
      const response = await fetch(`http://localhost:3000/api/shop-owner/${id}/permissions`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
      ...
      setOwners(prevOwners => 
        prevOwners.map(owner => 
          owner.id === id ? { ...owner, permissions: result.permissions } : owner
        )
      );
  ...
  const handleDeleteConfirm = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/shop-owner/${selectedOwner.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const response = await fetch(`http://localhost:3000/api/shop-owner/${id}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
                    ...
                    <tr key={owner.id}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <div className="flex items-center">
                          <button
                            onClick={() => handleStatusChange(owner.id, owner.status === 'active' ? 'inactive' : 'active')}
