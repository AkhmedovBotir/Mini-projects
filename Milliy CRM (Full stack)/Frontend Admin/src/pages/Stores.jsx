...
const Stores = () => {
  ...
  const admins = [
    ...
    { id: 2, name: 'Jane Smith', phone: '+998 90 987 65 43' },
    { id: 3, name: 'Bob Wilson', phone: '+998 90 456 78 90' }
  ];
  ...
  const handleStatusChange = async (storeId) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/shop/${storeId}/status`, {
        method: 'PATCH',
        headers: {
  ...
  const filteredStores = stores.filter(store => {
    ...
    const tariffMatch = !filters.tariff || store.tariff === filters.tariff;

    return searchMatch && statusMatch && tariffMatch;
  });
  ...
  const fetchOwners = async () => {
    try {
      ...
      const response = await fetch('http://localhost:3000/api/shop-owner/list', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      console.log('Shop owners:', result.owners || []);

      setOwners(result.shopOwners || []);
    } catch (error) {
  ...
  const handleDeleteStore = async (store) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/shop/${store.id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const handleEditStore = async (data) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/shop/${data.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
  ...
  const handleCreateStore = async (data) => {
    try {
      ...
      console.log('Request data:', {
        url: 'http://localhost:3000/api/shop/create',
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
                    ...
                    <tr key={store.id || index}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <Switch
                          ...
                          className={`${store.status === 'active' ? 'bg-blue-600' : 'bg-gray-200'
                        ...
                        >
                          <span
                            className={`${store.status === 'active' ? 'translate-x-5' : 'translate-x-0'
