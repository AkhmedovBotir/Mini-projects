...
const Departments = () => {
  ...
  const fetchDepartments = async () => {
    try {
      const response = await axios.post(`${API_URL}/department/filter?page=${currentPage}&size=${itemsPerPage}`, {
  ...
  const handleSaveDepartment = async () => {
    try {
      ...
      await axios.post(`${API_URL}/department`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleUpdateDepartment = async () => {
    try {
      ...
      await axios.put(`${API_URL}/department/${editingDepartment.id}`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const response = await axios.patch(
        `${API_URL}/department/${deletingDepartment.id}/wipe`,
        { visible: false },
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleStatusChange = async (department) => {
    try {
      ...
      await axios.put(
        `${API_URL}/department/${department.id}/status`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="p-4">
      <Helmet>
        ...
        <meta name="description" content="Admin panelga bo'limlar sahifasi" />
      </Helmet>
