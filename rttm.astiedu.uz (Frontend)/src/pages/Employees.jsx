...
const Employees = () => {
  ...
  const handleSaveEmployee = async () => {
    try {
      await axios.post(`${API_URL}/profile/registration`, newEmployee, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleUpdateEmployee = async () => {
    try {
      ...
      await axios.put(`${API_URL}/employee/${editingEmployee.id}`, payload, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      const response = await axios.put(
        `${API_URL}/employee/${deletingEmployee.id}/wipe`,
        { visible: false },  
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.status !== 200) {
        throw new Error('Xodimni o\'chirishda xatolik');
      }
  ...
  const handleStatusChange = async (employeeId, newStatus) => {
    ...
    try {
      await axios.put(
        `${API_URL}/employee/${employeeId}/status`,
        { status: newStatus },
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="p-4">
      <Helmet>
        ...
        <meta name="description" content="Admin panelga xodimlar sahifasi" />
      </Helmet>
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="mb-4">
          <div className="flex items-center space-x-4">
            ...
            {(searchTerm || filterRole || filterDepartment || filterPosition || filterStatus) && (
              <button
